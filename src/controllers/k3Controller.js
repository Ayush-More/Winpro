...
const rosesPlus = async (auth, money) => {
    ...
    // Update turn_over table for the user who placed the bet
    const sql_turnover = `
    ...
    `;
    await connection.execute(sql_turnover, [userInfo.phone, userInfo.code, userInfo.invite, money, money]);
...
const betK3 = async (req, res) => {
    try {
        ...
        // Check if user already has a bet in current period
        const currentPeriod = k3Now[0].period;
        const [existingBet] = await connection.query(
            'SELECT id FROM result_k3 WHERE phone = ? AND stage = ? AND game = ? AND status = 0',
            [user[0].phone, currentPeriod, game]
        );

        if (existingBet.length > 0) {
            return res.status(200).json({
                message: 'You already have a bet in this period. Only one bet per period is allowed.',
                status: false
            });
        }
        let userInfo = user[0];
...
const addK3 = async (game) => {
    try {
        ...
        await connection.execute(sql, [Number(period), 0, game, 0, timeNow]);
...
async function funHanding(game) {
    ...
    for (let i = 0; i < totalN; i++) {
        ...
        // Check for specific number wins
        if (lengWin.length >= 1) {
            isWin = true;
        }

        // Check for small (3-10)
        ...
        // Set final status based on win/loss
        if (isWin) {
...
const processLossRebate = async (userPhone, betAmount, timeNow) => {
    try {
        ...
        console.log(`K3 Loss rebate processed: ${rebateAmount} for phone ${userPhone}`);
    } catch (error) {
        console.error('Error processing K3 loss rebate:', error);
    }
};
