...
const processLossRebate = async (userPhone, betAmount, timeNow) => {
    try {
        ...
        console.log(`5D Loss rebate processed: ${rebateAmount} for phone ${userPhone}`);
    } catch (error) {
        console.error('Error processing 5D loss rebate:', error);
    }
};
...
const handling5D = async(typeid) => {
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        if (check) {
            ...
            let winnings = orders.price * 2;
            let fee = winnings * 0.02;
            let net = winnings - fee;
            nhan_duoc += net;
        }
