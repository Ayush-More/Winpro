...
function animationNewPar(data) {
    ...
    }, 500); // Reduced from 2500ms to 500ms for faster result display
}
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == true) {
                $('#money_show').text("₹ " + response.money.toFixed(2));

                // Show bet confirmation before clearing selections
                show5DBetConfirmation(join, list_join, value, x);
...
// Function to show 5D bet confirmation
function show5DBetConfirmation(join, list_join, money, x) {
    let betAmount = Number(money); // Show base bet amount like Wingo, not multiplied
    let totalAmount = Number(money) * Number(x); // Total amount for reference
    let gameType = '';
    let selectionText = '';

    // Determine game type and selection text
    if (join === 'total') {
        gameType = 'Sum';
        // For total bets, list_join contains the bet type (b/s/l/c)
        switch(list_join) {
            case 'b': selectionText = 'Big'; break;
            case 's': selectionText = 'Small'; break;
            case 'l': selectionText = 'Odd'; break;
            case 'c': selectionText = 'Even'; break;
            default: selectionText = list_join;
        }
    } else {
        // For A, B, C, D, E positions
        gameType = `Position ${join.toUpperCase()}`;
        ...
        if (list_join === 'b') {
            ...
            // Numbers selected
            selectionText = list_join.split('').join(', ');
        }
    }

    // Create confirmation message
    let confirmationHtml = `
        <div class="bet-confirmation-5d" style="
            position: fixed;
            top: 50%;
            left: 50%;
            ...
            min-width: 250px;
        ">
            <h4 style="margin: 0 0 10px 0; color: #4CAF50;">Bet Placed Successfully!</h4>
            <p style="margin: 5px 0;"><strong>Game Type:</strong> ${gameType}</p>
            <p style="margin: 5px 0;"><strong>Selection:</strong> ${selectionText}</p>
            <p style="margin: 5px 0;"><strong>Bet Amount:</strong> ₹${betAmount}</p>
            <p style="margin: 5px 0;"><strong>Multiplier:</strong> ${x}x</p>
            <p style="margin: 5px 0;"><strong>Total Cost:</strong> ₹${totalAmount}</p>
            <p style="margin: 10px 0 0 0; font-size: 12px; color: #ccc;">This message will disappear in 3 seconds</p>
        </div>
    `;
    ...
    setTimeout(() => {
        $('.bet-confirmation-5d').fadeOut(500, function() {
            $(this).remove();
        });
    }, 3000);
}
