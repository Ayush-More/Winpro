...
socket.on("data-server-trx", function(msg) {
    // Check if msg and msg.data exist and have content
    if (!msg || !msg.data || msg.data.length === 0) {
        console.log("Empty or invalid TRX data received");
        return;
    }

    // Only process TRX 1-minute game data
    if (msg.data[0].game != 'trx') return;

    $(".Loading").fadeIn(0);
    // Reduced delay for faster result display
    setTimeout(() => {
        ...
        console.log("TRX Data received:", data1);

        // Check if pre_block_hash exists before using it
        if (data1.pre_block_hash) {
            console.log("Pre block hash:", data1.pre_block_hash);
            showPreResults(data1.pre_block_hash);
        } else {
            console.log("No pre_block_hash found in data");
        }
        ...
        $(".Loading").fadeOut(0);
    }, 200); // Reduced from 1000ms to 200ms for faster result display
});
...
$(".game-list .con-box:eq(0) .page-nav .arr:eq(1)").click(function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            page_history = response.current_page || (pageno_history / 10 + 1);
            $(".game-list .con-box:eq(0) .page-nav .number").text(
                page_history + "/" + response.page
            );
            let list_orders = response.data.gameslist;
            $(".time-box .info .number").text(response.period);
            showPreResults(response.pre_block_hash);
...
$(".game-list .con-box:eq(0) .page-nav .arr:eq(0)").click(function(e) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(response) {
            if (response.current_page <= 1 || (pageno_history / 10 + 1) <= 1) {
...
$(".game-list .con-box:eq(1) .page-nav .arr:eq(1)").click(function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            page_my = response.current_page || (pageno_my / 10 + 1);
            $(".game-list .con-box:eq(1) .page-nav .number").text(
                page_my + "/" + `${(response.page) ? response.page : '1'}`
            );
            let list_orders = response.data.gameslist;
            $(".time-box .info .number").text(response.period);
            showPreResults(response.pre_block_hash);
...
$(".game-list .con-box:eq(1) .page-nav .arr:eq(0)").click(function(e) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(response) {
            if (response.current_page <= 1 || (pageno_my / 10 + 1) <= 1) {
...
$(".game-list .con-box:eq(2) .page-nav .arr:eq(1)").click(function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === false) {
                pageno_trend -= 10;
                $(".game-list .con-box:eq(2) .page-nav .arr:eq(1)").addClass("block-click");
                $(".game-list .con-box:eq(2) .page-nav .arr:eq(1)").removeClass("action");
                $(".game-list .con-box:eq(2) .page-nav .van-icon-arrow-right").css("color", "#7f7f7f");
                alertMessJoin(response.msg);
                return false;
            }
            $(".game-list .con-box:eq(2) .page-nav .arr:eq(0)").removeClass("block-click");
            $(".game-list .con-box:eq(2) .page-nav .arr:eq(0)").addClass("action");
            $(".game-list .con-box:eq(2) .page-nav .van-icon-arrow-left").css("color", "#fff");
            page_trend = response.current_page || (pageno_trend / 10 + 1);
            $(".game-list .con-box:eq(2) .page-nav .number").text(page_trend + "/" + response.page);
            let list_orders = response.data.gameslist;
            $(".time-box .info .number").text(response.period);
            showPreResults(response.pre_block_hash);
            showListOrder_t(list_orders, 2);
            loadTrend(list_orders, false);
        },
    });
});
$(".game-list .con-box:eq(2) .page-nav .arr:eq(0)").click(function(e) {
    e.preventDefault();
    $(".game-list .con-box:eq(2) .page-nav .arr:eq(1)").removeClass("block-click");
    $(".game-list .con-box:eq(2) .page-nav .arr:eq(1)").addClass("action");
    $(".game-list .con-box:eq(2) .page-nav .van-icon-arrow-right").css("color", "#fff");
    pageno_trend -= 10;
    let pageto = limit;
    $.ajax({
        type: "POST",
