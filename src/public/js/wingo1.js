...
socket.on("data-server", function (msg) {
  ...
  $(".Loading").fadeIn(0);
  // Remove delay for immediate result display
                        ...
                        <div data-v-a9660e98="" class="item c-row">
                            ...
                            <div data-v-a9660e98="" class="c-row c-row-between info">
                                <div data-v-a9660e98="">
                                    ...
                                    <div data-v-a9660e98="" class="issueName2" style="    color: #deba6d;
    font-size: 13px;"> Bet Amount - ₹${list_orders.amount} 
                                    </div>
                                </div>
                                        ...
                                        ${
                                          list_orders.status==1 && list_orders.bet ==0 
                                            ? '<span data-v-a9660e98="" class="success"> + ' +
                                              list_orders.money.toFixed(2).toFixed(2) * 4.5 +
                                              " </span>"
                                            :list_orders.status==1 && list_orders.bet ==5
                                            ? '<span data-v-a9660e98="" class="success"> + ' +
                                            list_orders.money.toFixed(2) * 4.5 +
                                            " </span>"
                                            : list_orders.status==1 && list_orders.result == 0 && list_orders.bet == 'd'
                                            ? '<span data-v-a9660e98="" class="success"> + ' +
                                            list_orders.money.toFixed(2) * 2 +
                                            " </span>"
                            ...
                            <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                ...
                                <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                    ...
                                    <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
                                        ...
                                        <div data-v-a9660e98="">${
                                          list_orders.money.toFixed(2) + list_orders.fee
                                        }</div>
                                    </div>
                                    <div data-v-a9660e98="" class="li c-row c-row-between c-row-middle">
...
$.ajax({
  ...
  success: function (response) {
    let list_orders = response.data.gameslist;
    // Don't override period from socket data - only set if not already set
    // if (!$(".time-box .info .number").text()) {
      $(".time-box .info .number").text(response.period);
    // }
    $(".game-list .con-box:eq(0) .page-nav .number").text("1/" + response.page);
    showListOrder(list_orders, 0);
...
window.onload = function() {
  function cownDownTimer() {
    // Synchronized countdown for 1-minute Wingo game
    setInterval(function() {
      ...
      var seconds1 = Math.floor(remainingSeconds / 10);
      var seconds2 = remainingSeconds % 10;
    ...
    setInterval(() => {
      ...
      // Play audio when 5 seconds or less remaining
      if (remainingSeconds <= 5 && remainingSeconds > 0) {
        if (clicked) {
          playAudio1();
        }
      }
      // Play audio when 1 second remaining
    ...
    setInterval(function() {
      ...
      // Block betting when 5 seconds or less remaining
