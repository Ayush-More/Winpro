...
function sendGame1() {
    ...
    // Prevent betting if too many options selected (more than 10 out of 14 total options)
    if (countwe.length > 10) {
        alertMess('Too many options selected. Please select fewer options to place bet.');
        $('.confirm').removeClass('block-click');
        return;
    }
...
function sendGame2() {
    ...
    // Prevent betting if too many options selected (more than 8 total combinations)
    if (count2.length + countwe.length > 8) {
        alertMess('Too many options selected. Please select fewer options to place bet.');
        $('.confirm').removeClass('block-click');
        return;
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                // Show bet confirmation
                showBetConfirmation('2 Same', listJoin, money, xvalue);
            }
            dropDown();
        }
    });
...
function sendGame3() {
    ...
    // Prevent betting if too many options selected (more than 5 out of 6 total options)
    if (countwe.length > 5) {
        alertMess('Too many options selected. Please select fewer options to place bet.');
        $('.confirm').removeClass('block-click');
        return;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                // Show bet confirmation
                showBetConfirmation('3 Same', listJoin, money, xvalue);
            }
            dropDown();
        }
    });
...
function sendGame4() {
    ...
    // Prevent betting if too many options selected (more than 8 total combinations)
    if (countwe.length + countwe3.length > 8) {
        alertMess('Too many options selected. Please select fewer options to place bet.');
        $('.confirm').removeClass('block-click');
        return;
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status) {
                ...
                // Show bet confirmation
                showBetConfirmation('Different', listJoin, money, xvalue);
            }
            dropDown();
        }
    });
...
// Function to show bet confirmation
function showBetConfirmation(gameType, betSelection, money, xvalue) {
    let betAmount = Number(money) * Number(xvalue);
    let selectionText = '';

    if (gameType === 'Total') {
        selectionText = betSelection;
    } else if (gameType === '2 Same' || gameType === '3 Same') {
        selectionText = betSelection.replace('@', ', ');
    } else {
        selectionText = betSelection;
    }
    ...
    let confirmationHtml = `
        <div class="bet-confirmation" style="
            position: fixed;
            top: 50%;
            left: 50%;
    ...
    setTimeout(() => {
        $('.bet-confirmation').fadeOut(500, function() {
            $(this).remove();
        });
    }, 3000);
}